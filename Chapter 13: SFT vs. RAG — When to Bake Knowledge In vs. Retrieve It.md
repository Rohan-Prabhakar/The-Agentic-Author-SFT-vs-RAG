...
Every architectural decision in an AI system moves value between four dimensions simultaneously: where knowledge is stored and how the system is organized; how queries are routed, retrieved, and generated at inference time; what the system costs and how fast it runs; and whether its failure modes are detectable before they cause harm. You cannot improve one dimension without accepting a tradeoff against at least one other. The SFT/RAG routing decision is a clean instance of this. Encoding knowledge in weights improves inference speed at the cost of making staleness failures invisible to standard evaluation. Routing to retrieval restores failure visibility and freshness but adds latency and token cost. Neither choice is wrong in the abstract. Both are wrong in the wrong context. The engineering discipline is in naming the tradeoff before deployment rather than discovering it from a deficiency notice six weeks later.
...
Volatility is not the only axis that determines the routing decision. Training cost, inference cost, hallucination risk, and deployment complexity all bear on the choice. Fine-tuning a 7B-parameter model runs roughly $500 to $2,000 per training run at mid-2024 cloud GPU pricing, a fixed cost that amortizes well at scale but is expensive to iterate on when knowledge changes. RAG adds approximately $0.015 per query in retrieval context, a variable cost that only makes sense when the retrieved knowledge is actually volatile. Parametric memory fails silently on point-lookup facts regardless of V, as the next section shows, which pushes precise numerical claims toward retrieval even when their volatility is low. A RAG system requires maintaining a vector store, an embedding pipeline, and a document ingestion process, and a fine-tuned model trades that operational complexity for a retraining pipeline instead. The V framework determines which axis dominates. When V is high, freshness overrides the others. When V is low, cost and complexity favor the weights.
...
Now take row five: Investment Adviser Act definitions. The statutory text of the 1940 Act has not been substantively amended in decades. A conservative estimate of $T_{\text{update}}$ is twenty years, giving $T_{\text{update}}$ divided by $T_{\text{retrain}}$ equal to 40, and V equal to 0.025. At any reasonable retraining cadence, this knowledge will be current in the weights when the model ships and will still be current when the model is retired. For categories like this, SFT is not just acceptable — it is the correct choice.

The decision boundary is not a bright line but a risk threshold. A reasonable default: any knowledge category with V greater than 1, meaning it updates faster than your retraining cycle, belongs in retrieval unless you can demonstrate that the cost of a stale answer is acceptable. What acceptable means in practice depends on the domain. In a compliance system, as the Chicago case shows, it means a deficiency notice and a client relationship damaged by a $10,000 error that the model produced with complete confidence.

### The SFT Failure Case: Chicago Compliance System
...
### The SFT-Preferred Case: Legal Research at Scale

Consider what the bottom three rows of the table 13.3 look like as a system in production. A law firm deploys a legal research assistant trained on the Investment Advisers Act of 1940, foundational fiduciary duty doctrine, and Form ADV filing structure. None of this knowledge has changed materially in decades. The V values for all three categories fall below 0.05. The system serves 50,000 queries per day from associates researching established legal standards. Routing this traffic through a RAG pipeline would cost approximately $750 per day in retrieval context alone, for knowledge the model already encodes correctly and reliably. The right architecture here is pure SFT: the model is trained once on the stable corpus, deployed, and serves queries at sub-200ms latency with no retrieval overhead until the underlying law changes, which the V framework predicts will not happen within any reasonable retraining cycle. This is the case the Chicago team handled correctly for five of their six knowledge categories. The error was not using SFT. It was using SFT for all six.

The opposite end of the spectrum is equally instructive. Some systems handle knowledge so volatile that no retraining cycle could ever keep pace with it, and the architectural decision is not a tradeoff but a hard constraint.

### The RAG-Required Case: 80 Days to Stay

The 80 Days to Stay system illustrates why some knowledge domains cannot be served by parametric memory at all, not because the model would fail to learn the facts, but because the facts change faster than any retraining cycle can track. The system was built to connect international students on expiring OPT and H-1B visas to funded startups willing to sponsor, using SEC Form D filings as its primary data source. Form D is a public regulatory filing that companies must submit when they raise capital: company name, address, funding amount, date. The dataset spans over 500,000 companies across ten years of quarterly filings, filtered down to approximately 30,000 verified, active targets by funding threshold, location, and H-1B sponsorship history cross-referenced against Department of Labor disclosure records.

Every data point in this system has a volatility index well above 1 relative to any reasonable model retraining cycle. A startup that raised five million dollars in Q3 may have spent it, pivoted, or shut down by Q1. A company that sponsored twelve H-1B applicants last year may have frozen hiring this quarter. A job listing scraped three weeks ago may have closed yesterday. If an LLM-powered search interface were built on top of this data, specifically a resume-to-company matching system of the kind the project's roadmap describes, the only viable architecture is RAG over a continuously updated document store. Encoding the funding records, sponsorship histories, or job listings into parametric weights would produce a system that confidently directs a student with 61 days remaining on their visa toward a company that no longer exists at the address the model remembers. The cost of a stale answer here is not a deficiency notice. It is a deportation deadline missed.

Both cases, the law firm and the visa search system, sit at opposite ends of the volatility spectrum. The Chicago system sat in the middle and got the routing wrong for one category. What follows examines what happens when the routing mechanism itself is unreliable.
...
The architectural contrast is worth making concrete. A mock SFT backbone frozen at November 2023 returns $15,000 for the volatile query about ADV filing penalties, with zero latency and zero retrieval cost. The hybrid system routes the same query to RAG and returns $25,000, sourced from the post-amendment document. A stable query about the legal definition of an investment adviser is classified as LOW_V with high confidence and routed to parametric memory at zero retrieval cost. The contrast between the two outputs is the architectural argument made visible: same model, same query phrasing, different storage routing, different answer, different consequence.
